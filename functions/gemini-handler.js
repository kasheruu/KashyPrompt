/** Shared Gemini /generate handler for Cloudflare Pages Functions and Workers + Assets. */
...
export function handleGenerateOptions() {
...
export async function handleGeneratePost(request, env) {
  try {
    ...
    if (!contentType.includes('application/json')) {
      return withCors(
        new Response(JSON.stringify({ error: 'Expected application/json' }), {
          status: 400,
    ...
    if (!env?.GEMINI_API_KEY) {
      return withCors(
        new Response(JSON.stringify({ error: 'Missing GEMINI_API_KEY secret on the server.' }), {
          status: 500,
          headers: { 'content-type': 'application/json' }
        })
      );
    ...
    if (!imageBase64 || !mimeType) {
      return withCors(
        new Response(JSON.stringify({ error: 'Missing imageBase64 or mimeType.' }), {
          status: 400,
          headers: { 'content-type': 'application/json' }
        })
      );
    ...
    return withCors(
      new Response(
        JSON.stringify({ error: lastError?.message || 'Failed to generate prompt with available models.' }),
        {
          status: 500,
          headers: { 'content-type': 'application/json' }
        }
